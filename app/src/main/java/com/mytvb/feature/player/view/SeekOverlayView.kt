...
import com.mytvb.feature.player.UiEvent
...
class SeekOverlayView @JvmOverloads constructor(
    ...
    init {
        val view = LayoutInflater.from(context).inflate(R.layout.seek_overlay_content, this, true)
        ...
        if (attrs != null) {
            val a = context.obtainStyledAttributes(attrs, R.styleable.SeekOverlayView, 0, 0)
            circleClipTapView.setAnimationDuration(
                a.getInt(R.styleable.SeekOverlayView_seekOverlay_animationDuration, 650).toLong()
            )
            seekSeconds = a.getInt(R.styleable.SeekOverlayView_seekOverlay_seekSeconds, 10)
            secondsView.setCycleDuration(
                a.getInt(R.styleable.SeekOverlayView_seekOverlay_iconAnimationDuration, 750).toLong()
            )
            circleClipTapView.setArcSize(
                a.getDimensionPixelSize(
                    R.styleable.SeekOverlayView_seekOverlay_arcSize,
                    context.resources.getDimensionPixelSize(R.dimen.px100)
                ).toFloat()
            )
            circleClipTapView.setCircleColor(
                a.getColor(
                    R.styleable.SeekOverlayView_seekOverlay_tapCircleColor,
                    ContextCompat.getColor(context, R.color.seek_overlay_tap_circle_color)
                )
            )
            circleClipTapView.setCircleBackgroundColor(
                a.getColor(
                    R.styleable.SeekOverlayView_seekOverlay_backgroundCircleColor,
                    ContextCompat.getColor(context, R.color.seek_overlay_background_circle_color)
                )
            )
            val textAppearance =
                a.getResourceId(R.styleable.SeekOverlayView_seekOverlay_textAppearance, R.style.SeekOverlaySecondsTextAppearance)
            TextViewCompat.setTextAppearance(secondsView.getTextView(), textAppearance)
            val iconRes = a.getResourceId(R.styleable.SeekOverlayView_seekOverlay_icon, R.drawable.ic_play_triangle)
            secondsView.m()
            secondsView.setIcon(iconRes)
            a.recycle()
        } else {
            circleClipTapView.setArcSize(context.resources.getDimensionPixelSize(R.dimen.px100).toFloat())
            circleClipTapView.setCircleColor(ContextCompat.getColor(context, R.color.seek_overlay_tap_circle_color))
            circleClipTapView.setCircleBackgroundColor(
                ContextCompat.getColor(context, R.color.seek_overlay_background_circle_color)
            )
            ...
            seekSeconds = 10
            TextViewCompat.setTextAppearance(secondsView.getTextView(), R.style.SeekOverlaySecondsTextAppearance)
        }
